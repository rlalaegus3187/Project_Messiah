...
- 코드 에이전트(AI) 활용
  - 구조 설명 후 코드 초안 생성
  - 직접 검토 및 수정 방식으로 개발 속도 개선 (전 프로젝트의 개발 시간이 4개월이었던 것에 비해 해당 프로젝트는 2달 개발 후 출시)
...
- 배틀 시, 단순히 객채로만 들고 있는 것이 아닌 Redis 사용
- 스크립트 및 보상 데이터 DB화 (형태가 일관적이지 않아 mapping 처리에 골머리)
- 배틀 객체 책임 단위 분리 및 구조 개선

---

본 저장소는 포트폴리오 공개를 목적으로, 보안 및 가독성을 고려해 전체 코드가 아닌 핵심 로직 위주로 구성되어 있습니다.
