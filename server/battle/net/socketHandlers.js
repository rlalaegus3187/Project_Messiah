...
function leavePlayerChannel(socket, playerId) {
  if (playerId == null) return;
  socket.leave(`p:${keyify(playerId)}`);
}

/** 레이드 룸 규칙 통일: battle:{teamId} */
...
export function installSocketHandlers(io) {
  io.on("connection", (socket) => {
    ...
    socket.on("team:leave", ({ room, chId } = {}) => {
      ...
      socket.leave(roomId);
      leavePlayerChannel(socket, playerId);
      ...
      if (raid.over) {
        ...
        socket.emit("raid:summary", {
          ...
          rewards: raid.rewards || {
            list: [],
            byPlayer: {},
            dungeonId: raid.map?.id || raid.map || "unknown",
          },
          state: {
    ...
    socket.on("input:move", ({ to }) => {
      ...
      for (const eff of effects) {
        ...
        // 타일 데미지(보스가 깐 장판이면 origin에 bossId가 들어갈 수 있음)
        if (dealtDmg > 0) {
          const origin = {
            by: eff?.origin?.bossId ?? "tile",
            action: trapSkill.id,
    ...
    socket.on("action:skill", ({ skillId, target }) => {
      ...
      if (skill.shape === "move" || skill.shape === "teleport") {
        ...
        io.to(currentRaid).emit("skill:move", {
          id: playerId,
          to: { x: p.x, y: p.y },
          ap: p.ap,
          skillId,
      ...
      if (skill.shape === "single") {
        ...
        for (let y = target.y - r; y <= target.y + r; y++) {
          for (let x = target.x - r; x <= target.x + r; x++) {
            if ((x - target.x) ** 2 + (y - target.y) ** 2 <= r * r) {
      ...
      for (const other of raid.players.values()) {
        ...
        const res = applyDamageToPlayer(
          ...
          other,
          dealtDmg,
          { by: playerId, action: skill.id, label: skill.name },
          ignoreDef
        );
      ...
      if (skillId !== "adrenaline" && raid.bosses instanceof Map) {
        for (const b of raid.bosses.values()) {
          ...
          const hitBoss = affectedTiles.some(
            (t) => t.x >= b.x && t.x < b.x + w && t.y >= b.y && t.y < b.y + h
          );
          if (!hitBoss) continue;
      ...
      if (Array.isArray(skill.applyStatus)) {
        for (const eff of skill.applyStatus) {
          ...
          if (eff.target === "self") {
            ...
            for (const other of raid.players.values()) {
              if (affectedTiles.some((t) => t.x === other.x && t.y === other.y)) {
                Status.add(other, spec);
              }
            }
